 ...
 */
var cachedLocalTimezoneOffsetMillis: Long? = null
...
expect fun writeExportFile(dir: String, filename: String, content: String): String
expect fun appendExportFile(path: String, content: String)

/** 将临时导出文件发布为最终文件；同名目标应被替换。用于「成功后才出现最终文件」。 */
expect fun renameExportFile(fromPath: String, toPath: String)

/** 删除导出文件；不存在时静默忽略。用于失败/取消时清理半成品。 */
expect fun deleteExportFile(path: String)

/** 列出目录下的文件绝对路径；不支持目录列举的平台返回空列表。 */
expect fun listExportFiles(dir: String): List<String>
...
val exportPublishLock = DshLock()
private val exportRegistryLock = DshLock()
private val activePartials = mutableSetOf<String>()
fun beginExport(filename: String) = exportRegistryLock.withLock {
    check(activePartials.add("$filename.partial")) { "同名文件正在导出，请稍后重试" }
}
fun endExport(filename: String) = exportRegistryLock.withLock { activePartials.remove("$filename.partial") }

fun databaseDiskBytes(path: String): Long =
 ...
 * 不会被回收。
 */
fun pruneExportCache(dir: String, keepPath: String) {
...
fun publishReadableExport(dir: String, filename: String, content: String, isCancelled: () -> Boolean): String {
...
expect fun shareExportFile(path: String)
