...
data class DshHostConnection(val baseUrl: String, val token: String = "")

/** 从 schemastery schema.toJSON()（{uid, refs}）中解析 object 字段的 union 常量选项。 */
fun dshParseSchemaChoices(schema: JSONObject?, field: String): List<DshSettingsChoice> {
...
fun dshSessionEventSummary(seq: Int, type: String, data: JSONObject): String {
...
fun dshSessionEventLevel(type: String, data: JSONObject): LogLevel = when {
 ...
 */
fun dshShouldLogSessionEvent(type: String, data: JSONObject): Boolean {
...
fun pendingInteractionRpcId(envelope: JSONObject, payload: JSONObject): String {
...
fun parseRespondReceipt(data: JSONObject): Pair<Boolean, String> {
...
fun dshEncodeQueryComponent(value: String): String {
