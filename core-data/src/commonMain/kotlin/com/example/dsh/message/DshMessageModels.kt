...
fun dshIsLiveAssistantText(message: DshMessage): Boolean =
 ...
 * shorter first-flush snapshot in [stored].
 */
fun dshDisplayedAssistantContent(
    stored: String,
    live: String,
    isLiveRow: Boolean,
): String {
 ...
 * text that precedes a tool call never becomes a footer tail.
 */
fun dshTurnTailAssistant(messages: List<DshMessage>): DshMessage? {
 ...
 * 中间过程性助手正文（后面还有同一回合的助手正文）不作为「最终回复」。
 */
fun dshShareSelectableIds(messages: List<DshMessage>): Set<String> {
 ...
 * 流式助手只挂到当前组，不单独成组；同一回合出现多段助手正文时，以最后一段作为最终回复。
 */
fun dshShareGroups(messages: List<DshMessage>): List<DshShareGroup> {
...
/** 某个消息所属的对话组；用户 Prompt 或其最终助手回复都能定位到同一组。 */
fun dshShareGroupForMessage(messages: List<DshMessage>, messageId: String): DshShareGroup? =
 ...
 * [isFirst] 标记当前消息是否为该过程块的首条；只有首条渲染摘要头，其余成员在折叠态隐藏。
 */
data class DshTurnProcessGroup(
    val key: String,
    val label: String,
 ...
 * 才分组，因此流式进行中的回合保持逐步可见。key 固定绑定 tail.id，展开状态跨重渲染稳定。
 */
fun dshTurnProcessGroup(messages: List<DshMessage>, message: DshMessage): DshTurnProcessGroup? {
 ...
 * be reused as the live streaming target.
 */
fun dshAssistantTailForCurrentTurn(messages: List<DshMessage>): DshMessage? {
    val lastUserIndex = messages.indexOfLast { it.role == DshMessageRole.USER }
...
/** Ignores id/streaming so a host key remap does not look like new content. */
fun DshMessage.visuallyEquals(other: DshMessage): Boolean =
    role == other.role &&
...
fun dshMessagesVisuallyEqual(left: List<DshMessage>, right: List<DshMessage>): Boolean {
    if (left.size != right.size) return false
    return left.indices.all { left[it].visuallyEquals(right[it]) }
}

data class DshContextCatalogEntry(
    val name: String,
    val description: String,
)

data class DshContextSection(
    val title: String,
    val body: String,
)

data class DshContextRecall(
    val label: String,
...
data class DshContextInstruction(
    val path: String,
    val action: String,
)

data class DshWebTimelineItem(
    val key: String,
    val kind: Kind,
    val text: String = "",
...
fun DshMessage.isRuntimeContextSnapshot(): Boolean {
