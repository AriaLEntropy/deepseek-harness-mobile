...
class DshDisclosureRowView : ComposeView<DshDisclosureRowAttr, ComposeEvent>() {
...
class DshDisclosureRowAttr : ComposeAttr() {
...
fun DshDisclosureRowAttr.dshExpandedPayload(): DshExpandedPayload = DshExpandedPayload(
    title = title,
    iconAsset = iconAsset,
    body = body,
    plainBody = plainBody,
 ...
 */
fun ViewContainer<*, *>.DshDisclosureBodyContent(
...
class DshLinkTextAttr : ComposeAttr() {
...
class DshLinkTextView : ComposeView<DshLinkTextAttr, ComposeEvent>() {
...
fun ViewContainer<*, *>.DshLinkText(init: DshLinkTextView.() -> Unit) {
    addChild(DshLinkTextView(), init)
}

private val dshUrlRegex = Regex("https?://[^\\s)\\]}>\"']+")

fun dshFirstUrl(text: String): String? = dshUrlRegex.find(text)?.value

/** 收起态的纯文本预览：保留前几行并加省略号，避免内部滚动。 */
fun dshPreviewText(text: String): String {
 ...
 */
fun dshContextDetailText(detail: DshContextDetail?): String {
 ...
 */
fun dshContextDetailPreview(detail: DshContextDetail?): String =
...
fun dshDisclosureCollapsible(attr: DshDisclosureRowAttr): Boolean {
    val text = dshDisclosureFullText(attr)
    return text.isNotBlank() && dshPreviewText(text) != text
}

/** 展开后正文是否超过限高、需要内部滚动（估算视觉行数，含换行折行）。 */
fun dshDisclosureScrollable(attr: DshDisclosureRowAttr): Boolean {
    val text = dshDisclosureFullText(attr)
...
fun ComposeView<*, *>.dshOpenLink(url: String) {
    val target = url.trim()
