...
internal enum class DshPluginFieldKind { TEXT, NUMBER, SECRET }

internal data class DshPluginConfigField(
    val key: String,
    val label: String,
    val hint: String,
...
internal data class DshPluginConfigCard(
...
private data class PluginFieldSpec(val key: String, val label: String, val hint: String, val kind: DshPluginFieldKind)

private val SHELL_FIELDS = listOf(
    PluginFieldSpec("timeoutMs", "命令超时（毫秒）", "单条命令允许运行多久，超时即终止。", DshPluginFieldKind.NUMBER),
    PluginFieldSpec("maxOutputBytes", "单流输出上限（字节）", "超出部分会转存到临时文件，而不是被丢弃。", DshPluginFieldKind.NUMBER),
)

private val AGENT_LOOP_FIELDS = listOf(
    PluginFieldSpec("maxParallelToolCalls", "并行工具调用数", "同一步内最多同时运行多少个可并行的调用。", DshPluginFieldKind.NUMBER),
)

private val WEB_SEARCH_FIELDS = listOf(
    PluginFieldSpec("apiKey", "API Key", "不写入设置文件。留空表示保持当前密钥。", DshPluginFieldKind.SECRET),
    PluginFieldSpec("baseURL", "接口地址", "留空则使用提供方默认地址。", DshPluginFieldKind.TEXT),
    PluginFieldSpec("maxUses", "单次请求最多搜索次数", "一次请求在必须作答前最多可以搜索多少次。", DshPluginFieldKind.NUMBER),
)

/**
 * 解析 `settings.describe` 的返回，投影为本部署可配置的三个 Host 插件卡。
 *
 * 与官方 web 的「设置 → 插件」区域一致：shell / agent-loop / web-search-deepseek。
 * 缺失的 namespace 直接不展示对应卡片。
 */
internal fun parseDshPluginConfig(value: JSONObject): DshPluginConfigState {
...
internal fun dshPluginValueText(value: Any?): String = when (value) {
    null -> ""
...
internal fun dshPluginSecretSet(namespace: JSONObject, pathSegment: String): Boolean {
