...
/** 录音浮层波形方块数量（滚动窗口）。 */
const val DSH_VOICE_WAVE_BARS = 32

/** 上滑取消阈值（dp）：手指距按下点向上超过该距离即进入取消态。 */
const val DSH_VOICE_CANCEL_THRESHOLD = 80f

/** 波形无新采样后的衰减系数。 */
const val DSH_VOICE_DECAY_FACTOR = 0.55f

/** 低于该值即视为静音，不再衰减。 */
const val DSH_VOICE_DECAY_EPSILON = 0.001f
 ...
 */
class DshVoiceWaveform(private val size: Int = DSH_VOICE_WAVE_BARS) {
...
fun dshVoiceCancelArmed(startPageY: Float, currentPageY: Float): Boolean =
    (startPageY - currentPageY) >= DSH_VOICE_CANCEL_THRESHOLD

/** 结束录音时取最终文本：优先原生 final，其次 partial，去空白。 */
fun dshVoiceResolvedText(committed: String, partial: String): String =
    committed.ifEmpty { partial }.trim()

/** 是否应把识别文本写入输入框（未取消且有内容）。 */
fun dshVoiceShouldCommit(cancelled: Boolean, text: String): Boolean =
