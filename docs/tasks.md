...
更新：2026-09-14。此表追踪本轮可确认的任务，代码完成与平台验收分别记录。9 月 11 日记录仅沿用已有 Task3 编号；9 月 12 日起按完整原始任务集追踪。

## 2026-09-14：日志增量游标与读取快照修复

- 增量查询按 `seq ASC` 读取游标后的最早一批，再倒序交给列表展示，避免突发超过 500 条时漏掉中间日志。单轮最多追赶 4 批，预算用尽后保留待追赶状态，下次轮询继续，即使没有新日志入队也能读完积压。
- `readPageSnapshot` 在同一存储锁内完成一次刷盘、级别聚合、可选目录、明细和全库最大序号读取。页面使用快照游标，避免级别筛选后的可见行序号导致重复累计，也避免统计与明细之间追加日志造成总数不一致。
- 清空/淘汰使用独立保留集版本触发重新对账；淘汰后即使压缩失败也使旧快照失效。空增量批次仅表示追赶完成或没有匹配项，不再重置已加载列表。
- 验证：隔离 JVM 探针直接调用编译后的生产 Kotlin 类，13 项通过，覆盖 600+10 条突发、整 500 条、2500 条跨轮追赶、级别筛选/空页、统计中入队、并发刷盘、清空、淘汰后压缩失败及取消。生产代码生成的 SQL 在内存 SQLite 中通过升降序、聚合、字面量关键词和完成哨兵检查。
- 构建：`:shared:compileDebugKotlinAndroid`、`:shared:compileKotlinJs`、`:androidApp:assembleDebug`、`:shared:compileKotlinOhosArm64` 均通过。iOS 编译和各端日志页真实交互待设备验证。

线程与页面回传沿用现有后台工作句柄 + Kuikly 页面定时回收结果的方式，参考 Kuikly `docs/DevGuide/set-timeout.md` 与 `docs/DevGuide/thread-and-coroutines.md`。
...
| 加分 | PDF/纯文本文件 | 🟡 | host-plugin backport：`/api/mobile-attachment/v1/upload` 写入会话工作目录并回传路径 handle；三端文档选择器 + 输入区草稿卡片 + 用户气泡文件卡片已接。待真机验收与旧 Host 实测 |
