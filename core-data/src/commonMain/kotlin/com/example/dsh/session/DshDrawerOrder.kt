...
/** 抽屉拖拽排序的对象类型。 */
enum class DshDrawerDragKind { NONE, SESSION, WORKSPACE }

/** 工作区拖拽使用的伪分组键，避免与会话的分组键（workspaceId）混淆。 */
const val DSH_WORKSPACE_DRAG_SCOPE = "__dsh_workspaces__"

/** 会话行固定高度：46dp，无额外行间距，布局与拖拽落点共用。 */
const val DSH_DRAWER_ROW_HEIGHT = 46f

const val DSH_DRAWER_WORKSPACE_ROW_HEIGHT = 40f
const val DSH_DRAWER_WORKSPACE_GROUP_GAP = 2f

/** 工作区文件夹行加分组上间距，用于拖拽落点估算。 */
const val DSH_DRAWER_WORKSPACE_HEADER_HEIGHT =
    DSH_DRAWER_WORKSPACE_ROW_HEIGHT + DSH_DRAWER_WORKSPACE_GROUP_GAP

/** 抽屉视图选项：分组方式，对齐电脑端 WorkspaceBrowser 的 groupBy。 */
const val DSH_DRAWER_GROUP_WORKSPACE = "workspace"
const val DSH_DRAWER_GROUP_FLAT = "flat"

/** 抽屉视图选项：排序方式，对齐电脑端 WorkspaceBrowser 的 orderBy。 */
const val DSH_DRAWER_ORDER_MANUAL = "manual"
const val DSH_DRAWER_ORDER_UPDATED = "updated"

const val DSH_PREF_DRAWER_SESSION_ORDER = "dsh.drawer.session.order"
const val DSH_PREF_DRAWER_WORKSPACE_ORDER = "dsh.drawer.workspace.order"
const val DSH_PREF_DRAWER_GROUP_BY = "dsh.drawer.group.by"
const val DSH_PREF_DRAWER_ORDER_BY = "dsh.drawer.order.by"

/** 当前正在进行的拖拽状态；`kind == NONE` 表示空闲。 */
data class DshDrawerDrag(
...
fun dshSessionOrderKey(scope: String): String = "$DSH_PREF_DRAWER_SESSION_ORDER.$scope"

fun dshWorkspaceOrderKey(scope: String): String = "$DSH_PREF_DRAWER_WORKSPACE_ORDER.$scope"

fun dshDrawerGroupByKey(scope: String): String = "$DSH_PREF_DRAWER_GROUP_BY.$scope"

fun dshDrawerOrderByKey(scope: String): String = "$DSH_PREF_DRAWER_ORDER_BY.$scope"

/** 按持久化的 id 顺序稳定重排；不在顺序中的元素保持原有相对顺序并排到最后。 */
fun <T> dshApplyOrder(items: List<T>, order: List<String>, idOf: (T) -> String): List<T> {
 ...
 */
fun dshRowDragOffset(state: DshDrawerDrag, scope: String, id: String, index: Int): Float {
...
fun dshDropIndex(offset: Float, startIndex: Int, heights: List<Float>): Int {
...
fun <T> dshMoveItem(items: List<T>, from: Int, to: Int): List<T> {
...
fun dshEncodeOrder(ids: List<String>): String {
    val array = JSONArray()
    ids.forEach { array.put(it) }
    return array.toString()
}

fun dshDecodeOrder(raw: String?): List<String> {
...
fun dshEncodeSessionOrder(order: Map<String, List<String>>): String {
    val obj = JSONObject()
...
fun dshDecodeSessionOrder(raw: String?): Map<String, List<String>> {
