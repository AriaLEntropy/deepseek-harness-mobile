...
fun contextSummary(source: JSONObject?): String {
...
fun toolInputSummary(value: Any?): String = when (value) {
    null -> ""
    is String -> value
    else -> value.toString()
}

fun toolOutputSummary(value: Any?): String = when (value) {
    null -> ""
    is String -> value
...
fun toolCardType(view: JSONObject): DshToolCardType = when (view.optString("card")) {
...
fun diffBody(view: JSONObject): String {
...
fun toolResultBody(type: DshToolCardType, view: JSONObject, fallback: String): String {
    return when (type) {
...
fun readBody(view: JSONObject): String {
...
fun searchBody(view: JSONObject): String {
...
fun webBody(view: JSONObject): String {
...
fun textFromBlocks(blocks: JSONArray?): String {
...
fun imagePreviewsFromBlocks(blocks: JSONArray?): List<String> {
...
fun attachmentIdsFromBlocks(blocks: JSONArray?): List<String> {
    if (blocks == null) return emptyList()
    val result = mutableListOf<String>()
    for (index in 0 until blocks.length()) {
        val block = blocks.optJSONObject(index) ?: continue
...
fun inlineImageDataUrl(block: JSONObject): String? {
