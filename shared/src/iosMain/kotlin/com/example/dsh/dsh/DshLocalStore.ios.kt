...
private class DshSqliteStore(path: String) : DshLocalStore {
    ...
    override fun saveSessions(sessions: List<DshSession>) = driver.transaction {
        ...
        sessions.forEach { s ->
            execute("INSERT OR REPLACE INTO dsh_sessions (id, title, workspace, updated_label, running, updated_at) VALUES (?, ?, ?, ?, ?, ?)", listOf(s.id, s.title, s.workspace, s.updatedLabel, if (s.running) "1" else "0", currentTimeMillis().toString()))
        }
    }
    ...
    override fun saveMessages(sessionId: String, messages: List<DshMessage>) = driver.transaction {
