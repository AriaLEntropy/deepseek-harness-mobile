...
data class DshSearchMatch(
    val lineNumber: Int,
    val line: String,
)

data class DshSearchFile(
    val path: String,
    val matches: List<DshSearchMatch>,
)

/** grep（分组命中）或 glob（路径列表）的结构化视图。 */
data class DshSearchCard(
...
fun dshParseWebCard(view: JSONObject?): DshWebCard? {
...
fun dshParseSearchCard(view: JSONObject?): DshSearchCard? {
    if (view == null) return null
