...
enum class DshTextExportPhase { IDLE, READING, WRITING, SHARING, READY, FAILED, CANCELLED }

data class DshTextExportState(
 ...
 * 已选计数放进底部弹窗，避免与底部重复。
 */
fun ViewContainer<*, *>.DshExportSelectionTopBar(
    totalCount: () -> Int,
    allSelected: () -> Boolean,
    onToggleAll: () -> Unit,
    onClose: () -> Unit,
...
const val DSH_EXPORT_CHECKBOX_SIZE = 20f

/**
 * 分享多选态的圆形勾选框：选中蓝底白勾，未选中浅色描边。
 * 不含点击事件，由外层容器决定点击行为（顶部栏整行、吸顶选择器自身）。
 */
fun ViewContainer<*, *>.DshExportCheckbox(
    selected: () -> Boolean,
    colors: () -> DshColorTokens,
 ...
 */
fun ViewContainer<*, *>.DshExportStickySelector(
    selected: () -> Boolean,
    onToggle: () -> Unit,
    colors: () -> DshColorTokens,
) {
 ...
 */
fun ViewContainer<*, *>.DshExportSelectionSheet(
    selectedCount: () -> Int,
...
fun ViewContainer<*, *>.DshExportRoundAction(
    label: () -> String,
    iconAsset: String,
    primary: Boolean,
    enabled: () -> Boolean,
...
/** 单个文件格式选项卡片：选中态蓝底描边。 */
fun ViewContainer<*, *>.DshExportFormatChip(
...
fun ViewContainer<*, *>.DshTextExportDialog(
