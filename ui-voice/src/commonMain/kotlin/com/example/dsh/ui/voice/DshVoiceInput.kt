...
const val DSH_VOICE_LEVEL_INTERVAL_MS = 90L

/** 松手后等待原生最终结果的兜底时长（毫秒）。 */
const val DSH_VOICE_STOP_TIMEOUT_MS = 900

/** 无新音量采样后开始衰减的间隔（毫秒）。 */
const val DSH_VOICE_DECAY_GAP_MS = 160L

/** 衰减刷新节拍（毫秒）。 */
const val DSH_VOICE_DECAY_TICK_MS = 90

/**
 * 语音输入相关文案。项目暂无全量 i18n 资源，这里按「语言」设置（settings.localeValue）
 * 在 zh / en 之间切换；其余 UI 文案仍为既有中文实现。
 */
data class DshVoiceStrings(
...
fun dshVoiceStrings(locale: String): DshVoiceStrings =
    if (locale == "en") DSH_VOICE_STRINGS_EN else DSH_VOICE_STRINGS_ZH

/** 把原生错误码解析为本地化文案；未知码回退原生 message，再回退通用失败文案。 */
fun dshVoiceErrorMessage(code: String, locale: String, fallback: String): String {
 ...
 */
fun ViewContainer<*, *>.DshVoiceRecordOverlay(
    visible: () -> Boolean,
    cancelArmed: () -> Boolean,
    partialText: () -> String,
    levels: () -> List<Float>,
